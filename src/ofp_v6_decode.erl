...
%%%
%%% @end
%%% Created : 13. Авг. 2018 10:27
%%%-------------------------------------------------------------------
-module(ofp_v6_decode).
-author("alex_shavelev").
...
do(Binary) ->
  ...
  case Length > byte_size(Binary) of
    ...
    false ->
      ...
      Type = ofp_v6_enum:to_atom(type, TypeInt),
...
decode_match_field(<<Header:4/bytes, Binary/bytes>>) ->
  <<ClassInt:16, FieldInt:7, HasMaskInt:1,
    Length:8>> = Header,
  Class = ofp_v6_enum:to_atom(oxm_class, ClassInt),
  HasMask = (HasMaskInt =:= 1),
  {Field, BitLength} = case Class of
                         openflow_basic ->
                           F = ofp_v6_enum:to_atom(oxm_ofb_match_fields, FieldInt),
                           {F, ofp_v6_map:tlv_length(F)};
                         _ ->
                           {FieldInt, Length * 8}
                       end,
  case HasMask of
...
decode_port_properties(<<TypeInt:16, Length:16, Rest/binary>>) ->
  Type = ofp_v6_enum:to_atom(port_desc_properties, TypeInt),
...
decode_properties(Binary, Properties) ->
  <<TypeInt:16, Length:16, _Pad:32,
    Data/bytes>> = Binary,
  Type = ofp_v6_enum:to_atom(queue_properties, TypeInt),
  case Type of
    min_rate ->
...
decode_bands(Binary, Bands) ->
  ...
  Type = ofp_v6_enum:to_atom(meter_band_type, TypeInt),
  case Type of
    drop ->
...
decode_actions(Binary, Actions) ->
  ...
  Type = ofp_v6_enum:to_atom(action_type, TypeInt),
  case Type of
    output ->
...
decode_instructions(Binary, Instructions) ->
  <<TypeInt:16, Length:16, Data/bytes>> = Binary,
  Type = ofp_v6_enum:to_atom(instruction_type, TypeInt),
  case Type of
    goto_table ->
...
table_feature_prop(Binary, Properties) ->
  ...
  Type = ofp_v6_enum:to_atom(table_feature_prop_type, TypeInt),
  IdsLength = Length - 4,
...
table_feature_prop_match(Binary, Ids) ->
  ...
  case Class of
    ...
    openflow_basic ->
      ...
      Id = ofp_v6_enum:to_atom(oxm_ofb_match_fields, IdInt),
...
decode_group_desc_stats(Binary) ->
  ...
  Type = ofp_v6_enum:to_atom(group_type, TypeInt),
...
decode_hello_elements(Binary, Acc) ->
  ...
  Type = ofp_v6_enum:to_atom(hello_elem, TypeInt),
  DataLength = Length - 4,
...
decode_body(error, Binary) ->
  ...
  Type = ofp_v6_enum:to_atom(error_type, TypeInt),
  case Type of
    experimenter ->
    ...
    _ ->
      ...
      Code = ofp_v6_enum:to_atom(Type, CodeInt),
...
decode_body(packet_in, Binary) ->
  ...
  Reason = ofp_v6_enum:to_atom(packet_in_reason, ReasonInt),
  ...
  #ofp_packet_in{buffer_id = BufferId, total_len = TotalLen,
    reason = Reason, table_id = TableId, cookie = Cookie,
...
decode_body(flow_removed, Binary) ->
  ...
  Reason = ofp_v6_enum:to_atom(flow_removed_reason, ReasonInt),
  Match = decode_match(MatchBin),
...
decode_body(port_status, Binary) ->
  ...
  Reason = ofp_v6_enum:to_atom(port_reason, ReasonInt),
  Port = decode_port(PortBin),
...
decode_body(flow_mod, Binary) ->
  ...
  Table = get_id(table, TableInt),
  BufferId = get_id(buffer_id, BufferIdInt),
  Command = ofp_v6_enum:to_atom(flow_mod_command, CommandInt),
...
decode_body(group_mod, Binary) ->
  ...
  Command = ofp_v6_enum:to_atom(group_mod_command, CommandInt),
  Type = ofp_v6_enum:to_atom(group_type, TypeInt),
  Group = get_id(group, GroupInt),
  Buckets = decode_buckets(BucketsBin),
...
decode_body(multipart_request, Binary) ->
  ...
  Type = ofp_v6_enum:to_atom(multipart_type, TypeInt),
  ...
  case Type of
    ...
    flow_monitor ->
      ...
      Cmd = ofp_v6_enum:to_atom(flow_monitor_command, CmdInt),
      Match = decode_match(MatchBin),
...
decode_body(role_request, Binary) ->
  ...
  Role = ofp_v6_enum:to_atom(controller_role, RoleInt),
...
decode_body(role_status, Binary) ->
  ...
  Role = ofp_v6_enum:to_atom(controller_role, RoleInt),
  Reason = ofp_v6_enum:to_atom(controller_role_reason, ReasonInt),
...
decode_body(table_status, Binary) ->
  ...
  Reason = ofp_v6_enum:to_atom(table_reason, ReasonInt),
...
decode_body(bundle_control, Binary) ->
  ...
  Type = ofp_v6_enum:to_atom(bundle_ctrl_type, TypeInt),
...
%%
%% It converts the type to an atom using ofp_v6_enum:to_atom, and
...
extract_properties(Enum, <<TypeInt:16, Length:16, Rest/binary>>) ->
  Type = ofp_v6_enum:to_atom(Enum, TypeInt),
...
decode_flow_updates(<<Length:16, EventInt:16, Bin/bytes>>) ->
  Event = ofp_v6_enum:to_atom(flow_update_event, EventInt),
  L = Length - 4,
...
binary_to_flags(Type, Binary) ->
  ofp_utils:binary_to_flags(ofp_v6_enum, Type, Binary).

get_id(Enum, Value) ->
  ofp_utils:get_enum_name(ofp_v6_enum, Enum, Value).
